  ...
  if (FLAGS_type == 0) {
    ...
    for (size_t i = 0; i < task_num; i++) {
      ...
      thread_pool.SubmitSync([i, input_size, input_data, path, &dst_data_size,
        ...
        auto output_size = output_data.size();
        ...
        // write back to disk
        std::ofstream file(path + std::to_string(i) + ".gzip.bin",
                           std::ios::binary);
        file.write(output_data.c_str(), output_size);
    ...
    for (size_t i = 0; i < task_num; i++) {
      ...
      thread_pool.SubmitSync([i, path, output_data, output_size,
