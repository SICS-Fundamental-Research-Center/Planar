#ifndef GRAPH_SYSTEMS_NVME_IO_IO_URING_READER_H
#define GRAPH_SYSTEMS_NVME_IO_IO_URING_READER_H

#include <fcntl.h>
#include <liburing.h>
#include <sys/ioctl.h>

#include <filesystem>
#include <fstream>
#include <list>
#include <memory>
#include <string>
#include <utility>

#include "core/common/blocking_queue.h"
#include "core/common/config.h"
#include "core/data_structures/buffer.h"
#include "core/data_structures/graph_metadata.h"
#include "core/data_structures/serialized.h"
#include "nvme/data_structures/graph/block_csr_graph.h"
#include "nvme/data_structures/memory_buffer.h"

namespace sics::graph::nvme::io {

struct io_data {
  core::common::BlockID block_id;
  size_t size;
  size_t offset;
  char* addr;
  bool segment = false;
};

std::vector<int> Fds;

// io_uring to read
class CSREdgeBlockReader {
 private:
  using OwnedBuffer = sics::graph::core::data_structures::OwnedBuffer;
  using Serialized = sics::graph::core::data_structures::Serialized;
  using BlockID = core::common::BlockID;
  using GraphID = core::common::GraphID;
  using VertexID = core::common::VertexID;

 public:
  CSREdgeBlockReader() = default;
  ~CSREdgeBlockReader() { io_uring_queue_exit(&ring_); }

  void Init(const std::string& root_path, data_structures::EdgeBuffer* buffer) {
    // copy the root path
    root_path_ = root_path;
    buffer_ = buffer;
    meta_ = buffer_->GetMetaPtr();
    auto ret = io_uring_queue_init(QD, &ring_, 0);
    if (ret < 0) {
      LOGF_FATAL("queue_init: {}", ret);
    }
    blocks_addr_.resize(meta_->num_subBlocks);
  }

  static int get_file_size(int fd, off_t* size) {
    struct stat st;
    if (fstat(fd, &st) < 0) return -1;
    if (S_ISREG(st.st_mode)) {
      *size = st.st_size;
      return 0;
    } else if (S_ISBLK(st.st_mode)) {
      unsigned long long bytes;
      if (ioctl(fd, BLKGETSIZE64, &bytes) != 0) return -1;
      *size = bytes;
      return 0;
    }

    return -1;
  }

  void Read(std::vector<BlockID> blocks_to_read) {
    for (int i = 0; i < blocks_to_read.size(); i++) {
      auto bid = blocks_to_read.at(i);
      auto path = root_path_ + "_blocks/" + std::to_string(bid) + ".bin";
      Fds[bid] = open(path.c_str(), O_RDONLY);  // TODO: add O_DIRECT flag
      auto fd = Fds[bid];
      struct stat sb;
      if (fstat(fd, &sb) < 0) {
        LOG_FATAL("Error at fstat");
      }
      io_data* data = (io_data*)malloc(sizeof(io_data));
      data->size = sb.st_size;
      data->block_id = bid;
      data->addr = (char*)buffer_->GetGraphPtr()->ApplySubBlockBuffer(bid);
      data->offset = 0;
      struct io_uring_sqe* sqe;
      sqe = io_uring_get_sqe(&ring_);
      if (!sqe) {
        free(data);
        buffer_->GetGraphPtr()->Release(bid);
        LOG_FATAL("Error at get sqe");
      }
      io_uring_prep_read(sqe, fd, data->addr, data->size, data->offset);
      io_uring_sqe_set_data(sqe, data);
    }
    auto ret = io_uring_submit(&ring_);
    if (ret < 0) {
      LOG_FATAL("Error at submit sqes");
    }
  }

  void Read(io_data* data) {
    auto fd = Fds[data->block_id];
    struct io_uring_sqe* sqe;
    sqe = io_uring_get_sqe(&ring_);
    if (!sqe) {
      free(data);
      LOG_FATAL("Error at get sqe");
    }
    io_uring_prep_read(sqe, fd, data->addr, data->size, data->offset);
    io_uring_sqe_set_data(sqe, data);
    auto ret = io_uring_submit(&ring_);
    if (ret < 0) {
      LOG_FATAL("Error at submit sqes for read short");
    }
  }

  size_t GetBlockReady() {
    struct io_uring_cqe* cqe;
    size_t num_cqe = 0;
    std::string ids = "";
    while (true) {
      auto ret = io_uring_peek_cqe(&ring_, &cqe);
      if (ret != 0) {
        break;
      }
      io_data* data = (io_data*)io_uring_cqe_get_data(cqe);
      auto size_to = data->size;
      if (size_to > 2747483647) {
        auto size = cqe->res;
      } else {
        auto size = cqe->res;
        if (size != data->size) {
          if (cqe->res > 0) {
            data->addr = data->addr + size;
            data->size = data->size - size;
            if (data->size < 0) {
              LOG_FATAL("size less 0");
            }
            data->offset = data->offset + cqe->res;
            io_uring_cqe_seen(&ring_, cqe);
            Read(data);
            continue;
          } else {
            LOGF_FATAL(
                "Read error in edge block, size is not fit! error code {}, "
                "sub_blockID {}, subgraphID {}",
                cqe->res, data->block_id);
          }
        }
      }
      // Set address and <gid, bid> entry for notification.
      //      graphs_->at(data->gid).SetSubBlock(data->block_id,
      //      (uint32_t*)data->addr);
      buffer_->LoadFinish(data->block_id);
      buffer_->AccumulateRead(data->size);
      io_uring_cqe_seen(&ring_, cqe);
      ids += std::to_string(data->block_id) + " ";
      close(Fds[data->block_id]);
      free(data);
      num_cqe++;
    }
    //    if (ids != "") {
    //      LOGF_INFO("Read blocks: {}", ids);
    //    }
    return num_cqe;
  }

  size_t SizeOfReadNow() { return read_size_; }

  // TODO: use queue to replace vector?
  std::vector<BlockID> GetReadBlocks() {}

  // Get the block address for iterate edges.
  VertexID* GetBlockAddr(BlockID bid) { return blocks_addr_.at(bid); }

  // Release the block address after using.
  void ReleaseBlockAddr(BlockID bid) { blocks_addr_.at(bid) = nullptr; }

  bool IsFinish() { return !io_uring_cq_has_overflow(&ring_); }

 private:
  std::string root_path_;

  core::data_structures::BlockMetadata* meta_;
  struct io_uring ring_;
  data_structures::EdgeBuffer* buffer_;

  std::queue<io_data*> reload_ids_;
  std::vector<VertexID*> blocks_addr_;  // to delete.

  size_t read_size_ = 0;  // use MB
};

}  // namespace sics::graph::nvme::io

#endif  // GRAPH_SYSTEMS_NVME_IO_IO_URING_READER_H
