...
VertexID VertexCutPartitioner::GetBucketID(VertexID vid,
                                           VertexID n_bucket) const {
  return fnv64_append_byte(vid, 3) % n_bucket;
}

void VertexCutPartitioner::RunPartitioner() {
  ...
  auto size_per_bucket = (VertexID*)malloc(sizeof(VertexID) * n_partitions_);
  memset(size_per_bucket, 0, sizeof(VertexID) * n_partitions_);
  auto max_vid_per_bucket = (VertexID*)malloc(sizeof(VertexID) * n_partitions_);
  memset(max_vid_per_bucket, 0, sizeof(VertexID) * n_partitions_);
  auto min_vid_per_bucket = (VertexID*)malloc(sizeof(VertexID) * n_partitions_);
  VertexID max_vid = 0, min_vid = MAX_VERTEX_ID;
  for (VertexID i = 0; i < n_partitions_; i++)
    min_vid_per_bucket[i] = MAX_VERTEX_ID;

  memset(min_vid_per_bucket, 0, sizeof(VertexID) * n_partitions_);
  ...
  auto edge_bucket = (VertexID**)malloc(sizeof(VertexID*) * n_partitions_);
  ...
  auto buckets_offset = (VertexID*)malloc(sizeof(VertexID) * n_partitions_);
  memset(buckets_offset, 0, sizeof(VertexID) * n_partitions_);
